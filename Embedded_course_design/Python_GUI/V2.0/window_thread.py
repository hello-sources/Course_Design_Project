...
def Wet_Modify():
    tk.messagebox.showwarning('警告', '不能修改传感器湿度')

def Pre_Modify():
    tk.messagebox.showwarning('警告', '不能修改传感器压强值')
...
def Connect():
    ...
    tk.messagebox.showinfo('提示', '串口已经打开')
...
def SendMsg(pat):
    pat1 = str(pat)
    result = ser.write((pat1).encode("utf-8"))
    print("写总字节数", result)
   
def Close():
    ser.close()
    tk.messagebox.showinfo('提示', '串口已经关闭')
...
filemenu.add_command(label='Exit', command=lambda : Mythread(window.quit))
...
a = tk.Button(window, text = '请选择工作模式', font=('Arial', 12), width=20, height=1, command=(SendMsg, pat))
a.pack()
...
b = tk.Button(frame_l, text = '修改湿度', font=('Arial', 12), width=10, height=1, command=lambda : Mythread(Wet_Modify)).pack()
c = tk.Button(frame_r, text = '修改压强', font=('Arial', 12), width=10, height=1, command=lambda : Mythread(Pre_Modify)).pack()

def selWet(event):
    tk.messagebox.showinfo('提示', '获取到的湿度值是' + cb_wet.get())

var_wet = tk.StringVar()
cb_wet = ttk.Combobox(frame_l, textvariable=var_wet, width = 12)
cb_wet.pack()
cb_wet['value'] = ('10%', '20%', '30%', '40%', '50%', '60%', '70%', '80%')
cb_wet.current(0)

cb_wet.bind("<<ComboboxSelected>>", selWet)

def selPre(event):
    tk.messagebox.showinfo('提示', '获取到的压强值是' + cb_pre.get())

var_pre = tk.StringVar()
cb_pre = ttk.Combobox(frame_r, textvariable=var_pre, width = 12)
cb_pre.pack()
cb_pre['value'] = ('1hpa', '2hpa', '3hpa', '4hpa', '5hpa')
cb_pre.current(0)

cb_pre.bind("<<ComboboxSelected>>", selPre)

d = tk.Button(frame_l, text = '打开串口', font=('Arial', 12), width=10, height=1, command=Connect).pack()
e = tk.Button(frame_r, text = '关闭串口', font=('Arial', 12), width=10, height=1, command=Close).pack()

f = tk.Button(window, text = '获取信息', font=('Arial', 12), width=10, height=1, command=GetMessage).pack()
