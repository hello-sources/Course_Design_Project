  ...
  <h1 style="text-align: right"><span id="show"></span></h1>
     ...
     <c:forEach items="${questions}" var="q">
       <tr>
         <td>${q.qid }</td>
         <td>${q.title }</td><td>
       <input type="radio" name="a${q.qid }" value="A"/>${q.optionA }<br/>
       <input type="radio" name="a${q.qid }" value="B"/>${q.optionB }<br/>
       <input type="radio" name="a${q.qid }" value="C"/>${q.optionC }<br/>
       <input type="radio" name="a${q.qid }" value="D"/>${q.optionD }</td>
       <td>分数:${q.score }</td>
       </tr> 
     </c:forEach>
     <tr align="center">
